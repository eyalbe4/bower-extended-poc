...
function getConstructor(source, config, registryClient) {
    var pkgResolver;
    var registryPrefix = extractResolverPrefix(source);
    registryPrefix = "artifactory";
    if(registryPrefix) {
        try {
            pkgResolver = require('bower-' + registryPrefix + '-resolver');
...
function extractResolverPrefix(source){
    var match;
    match = source.split(":");
